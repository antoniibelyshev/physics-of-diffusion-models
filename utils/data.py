...
class HuggingFaceDataset(Dataset[tuple[Tensor, ...]]):
    """
    A generic class to wrap Hugging Face datasets into a PyTorch Dataset.
    """
    def __init__(self, hf_dataset_name: str, image_size: tuple[int, ...], *, train: bool = True):
        super().__init__()
...
def get_dataset(config: Config, train: bool = True) -> Dataset[tuple[Tensor, ...]]:
    ...
    if (hf_dataset_name := dataset_config.hf_dataset_name) is not None:
        obj_size = dataset_config.obj_size
        return HuggingFaceDataset(hf_dataset_name, obj_size, train=train)
    else:
